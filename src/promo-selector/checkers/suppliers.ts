...
// Process-wide, keyed by `${supplierId}:${userId}:${authenticated}`, 60s TTL.
// The auth flag is part of the key: the loaders branch on it (guests get neutral
// defaults), so a guest hit must never be served to the same userId once they
// log in within the TTL — and vice versa. Because impressions are part of the
// cached userData, an impression recorded via POST /impressions is not
// reflected in limit/cooldown for up to TTL on this instance. Acceptable:
...
async function cached<T>(supplierId: SupplierId, userId: string, authenticated: boolean, load: () => Promise<T>): Promise<T> {
  const nowMs = Date.now();
  const key = `${supplierId}:${userId}:${authenticated ? 1 : 0}`;
...
function loadUserData(userId: string, authenticated: boolean, deps: SupplierDeps): Promise<UserData> {
  return cached('userData', userId, authenticated, async () => {
...
function loadListingStats(userId: string, authenticated: boolean, deps: SupplierDeps): Promise<ListingStats> {
  ...
  return cached('listingStats', userId, authenticated, () => deps.listingService.getListingStats(userId));
}

 ...
 * matrix changes. `promoline` is deliberately NOT in this desktop-only list: the
 * renderer ships the format natively for every device (touch and desktop alike —
 * the promoline row lives in every catalogue feed), so it belongs to neither
 * side of the matrix here. On touch these formats render to nothing, so we drop such promos
